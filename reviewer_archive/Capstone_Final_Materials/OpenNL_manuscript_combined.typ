...
($upright("SD") < 0.025$ SII) for any fixed parameter set. Crucially, a
sensitivity analysis reveals that the optimizer actively collides with
these mathematical boundary conditions. Because the objective function
is dominated by physiological loudness constraints, modulating the
underlying heuristic triggers yielded exceptionally tight response
envelopes. This indicates that once the optimizer reaches the active
penalty boundary, the underlying heuristic parameters are rendered
largely irrelevant, effectively preventing runaway amplification but
emphasizing the need to include binding penalty parameters in future
sensitivity sweeps.
...
A2, A3, and A5 effectively dictate the final modeled loudness to within
0.1 sones. Because the objective function is entirely bound by these
hard distortion penalties, modulating the underlying heuristic triggers
(such as the base anchor or slope bypass) yielded minimal variance
(e.g., A5 fluctuating narrowly between 4.26 and 4.37 sones). This
indicates that once the active penalty wall is reached, the underlying
heuristic parameters are rendered practically irrelevant. While this
successfully prevents the massive runaway amplification typical of
historically unregularized intelligibility optimization, any future
claims of comprehensive parameter robustness must be tested against a
sweep that formally includes these binding penalty variables.
