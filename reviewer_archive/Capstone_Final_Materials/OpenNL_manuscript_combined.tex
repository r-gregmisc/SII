...
with a massive linear penalty weight (\(\lambda_{loud} = 2000\)) on
...
in linear sones---a figure that explicitly represents the sone-domain
equivalent of a 13 dB level difference under the standard
doubling-per-10-dB relation, rather than a directly measured loudness
ratio (Denk et al., 2025; Moore et al., 2014; Oetting et al., 2016,
...
Uncalibrated midpoint balancing half-gain rules and preference data. \\
...
\textbf{TABLE II. Variance-Partitioning of Heuristic Parameters (65 dB
SPL Input).} \emph{Note: Generated using the aggressive 60 dB HL booster
onset. The \(\eta^2\) values partition the variance across the
deterministic simulator's parameters.}
...
Profile
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Median ANSI SII {[}Min, Max{]}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Median Loudness {[}Min, Max{]}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Primary Variance Partition (\(\eta^2\))
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\textbf{A2} & 0.89 {[}0.89, 0.89{]} & 5.06 {[}5.06, 5.06{]} & None
(Mathematically Bound) \\
\textbf{A4} & 0.58 {[}0.58, 0.59{]} & 5.22 {[}5.22, 5.22{]} & Anchor
(72.2\%) \\
\textbf{A5} & 0.60 {[}0.56, 0.65{]} & 5.25 {[}5.25, 5.25{]} & Anchor
(55.9\%) \\
\end{longtable}
...
dominate the objective landscape, yet still critically rely on heuristic
triggers to dictate the final optimization path. The objective function
weights audibility against a normalized 0-100 scale, while applying a
massive linear penalty for violating the physiological loudness ceiling.
The tight response envelope for physiological loudness serves as
empirical confirmation that the derivative-free Nelder-Mead solver
successfully navigates the non-smooth penalty landscape to arrive
exactly at the mathematically dictated limit without numerical
divergence. In fact, for all three topologically unstable profiles, the
resulting physiological loudness was locked to within \(<0.01\) sones
across all 768 permutations.

However, while the physiological loudness is mathematically bound by the
penalties, the resulting audibility (ANSI SII) is not. Once the solver
collides with the physiological loudness ceiling, it must slide along
the multidimensional edge of this constraint boundary to maximize
audibility without violating the 3.0:1 Compression Ratio projection. As
shown for Profile A5, depending on the starting conditions dictated by
the underlying heuristic \texttt{Anchor} parameter, the solver finds
significantly different local maxima along this boundary edge (yielding
ANSI SII scores ranging from 0.56 to 0.65, despite the loudness
remaining perfectly locked at 5.25 sones). This demonstrates that the
optimization space is a complex, interlocking web of constraints: the
hard algebraic penalties mathematically guarantee physiological safety
(e.g., loudness), but the heuristic triggers remain strictly necessary
to guide the solver toward the most theoretically optimal audibility
within that safe space. This definitively answers the architectural
question of WDRC optimization: heuristic seeds are not replaced by
objective functions; rather, they are required to successfully navigate
the highly non-convex topography created by physiological constraints.
...
3 profiles) for clinical profiles A2 (Reverse Slope), A4 (Severe), and
A5 (Profound), illustrating how the active physiological loudness
...
(\(\pm 10\) dB jitter) across Profiles A4, A2, and A5. By utilizing an
adaptive Nelder-Mead simplex (Gao \& Han, 2012) scaled to the
...
growth. - \textbf{Robust Convergence and Target Mapping (A1, A2)}: With
...
\textbf{Inefficient Objective Exploitation (A6, A7)}: For profile A6,
...
Frontiers}: As detailed in Section II.A, applying fixed normal-hearing
scalars to monaural outputs fails catastrophically for impaired
listeners (van Beurden et al., 2021; Pieper et al., 2021). An
optimization routine operating beneath a monaural ceiling (e.g., 4.32
sones for profile A5) appears mathematically safe in isolation, yet
predictably collapses into acute acoustic intolerance when fitted
bilaterally due to excess summation. Because applying a fixed scalar to
...
(\textbf{Figure 2}). While this cross-algorithmic MAE of 0.39 sones
...
\textasciitilde80 dB HL regardless of prescribed gain. This tension
underscores why Open-NL's mathematically rigid penalty enforcement
naturally prevents dangerous over-amplification in profound losses
without requiring manual heuristic overrides.
...
subtracting the air-bone gap (\(J_i\)), and departs from common ANSI
S3.5 implementations by restricting the internal cochlear noise floor
calculation strictly to sensorineural loss
(\(X'_i = X_i + \max(0, T'_i - J_i)\)), on the rationale that conductive
attenuation should not falsely inflate internal noise. While the rigid
...
as a rating of 6 (``Very loud'') or 7 (``Too loud'') on the English
7-point Categorical Loudness Scaling (CLS) procedure (ISO
16832:2006)---when presented with continuous broadband speech (e.g.,
ISTS) at 65 and 80 dB SPL, relative to a matched NAL-NL2 baseline.
Empirically quantifying this rejection threshold will provide the
ground-truth data required to constrain distortion-aware objective
functions in future stochastic calibrations.
...
\(\lambda_{loud} = 2000\)) explicitly override clinical rules via
...
binding penalty parameters themselves (\texttt{cap\_knots},
\(\lambda_{loud}\), \(\lambda_{cr}\), and the CR ceiling). Subjecting
these foundational algorithmic hard-stops to global sensitivity indexing
will transform the current analytical identities into fully calibrated,
empirically robust clinical constraints.
...
\item
  ...
  \(\mathbf{PTA}_{knots} = [10, 35, 50, 75, 90]\) dB HL from
  ...
  \item
    \(L_{in} = 65\) dB SPL:
    \(\mathbf{K}_{65} = [7.0, 5.0, 5.5, 6.5, 6.5]\) sones
  \item
    \(L_{in} = 80\) dB SPL:
