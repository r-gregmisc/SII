...
open_nl <- function(speech = 65, threshold, freq, ..., 
  ...
  if (optimize) {
    ...
    gain_65 <- calculate_open_nl_gain(freq=freq, threshold=threshold, input_level=65, gender=gender, experience=experience, config=config, coupling=coupling, module=module, ldl=ldl, loss=loss, distortion_category=distortion_category, user_cr=user_cr, abg_fraction=abg_fraction, enable_severe_booster=enable_severe_booster, booster_onset=booster_onset, disable_sdlfp=disable_sdlfp, ...)
    ...
    obj_fn <- function(shifts) {
      # Add a massive penalty for shifts outside +10 / -60 dB to anchor to clinical heuristic
      out_of_bounds_penalty <- (sum(pmax(0, shifts - 10)^2) + sum(pmax(0, -shifts - 60)^2)) * 1000.0
      ...
      fi_6 <- freq
      Ei_6 <- speech_spec_65 + gain_array
      dense_l <- approx(x = log10(fi_6), y = Ei_6, xout = log10(dense_f), rule = 1)$y
      
      idx_low <- which(dense_f < fi_6[1])
      if (length(idx_low) > 0) dense_l[idx_low] <- Ei_6[1] - 24 * log2(fi_6[1] / dense_f[idx_low])
      idx_high <- which(dense_f > fi_6[length(fi_6)])
      if (length(idx_high) > 0) dense_l[idx_high] <- Ei_6[length(Ei_6)] - 24 * log2(dense_f[idx_high] / fi_6[length(fi_6)])
      ...
      if (!is.null(loud_res)) {
        ...
        if (is_reverse_slope) {
            dynamic_cap <- max(4.0, 0.5 + 0.08 * pta_sn_local)
        } else {
            # Targeting ~5.25 sones in Band SPL for A1 (PTA ~42.5), which maps to ~8 binaural sones natively
            dynamic_cap <- max(5.0, 1.0 + 0.10 * pta_sn_local)
        }
    ...
    cat("--- OPTIMIZER STARTING IN OPEN_NL ---\n", file=stderr())
    cat("Initial Heuristic Gain:", paste(round(final_gain_65, 2), collapse=", "), "\n", file=stderr())
    ...
    cat("Optimizer Best Shifts:", paste(round(best_shifts, 2), collapse=", "), "\n", file=stderr())
    ...
    constrained_best_shifts <- pmax(-60, pmin(10, best_shifts))
    
    # Store convergence stats for potential upstream analysis
    attr(gain, "convergence") <- convergence_stats
