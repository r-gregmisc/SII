...
server <- function(input, output, session) {
  ...
  sii_obj <- reactive({
    ...
    if (preset %in% c("a1", "a2", "a3", "a4", "a5", "a6", "a7") && isTRUE(input$prescription == "Open-NL")) {
      obj$target_nalnl2 <- SII:::get_jd2011_target(preset, "NAL-NL2", d$f_21, target_level)
    }
  ...
  output$comparison_table <- renderTable({
    ...
    calc_loudness <- function(obj) {
      if (input$config == "unilateral") {
        ...
        return(SII:::calculate_binaural_loudness(obj, obj))
      }
    }
    ...
    if (preset %in% c("a1", "a2", "a3", "a4", "a5", "a6", "a7")) {
      target_nalnl2 <- SII:::get_jd2011_target(preset, "NAL-NL2", d$f_21, target_level)
  ...
  output$compression_prescription <- renderUI({
    ...
    comp_presc <- SII:::prescribe_compression(freq = f_htl, threshold = threshold, module = input$module)
