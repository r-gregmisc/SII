...
`cap_knots`\; Section S.I.12). For severe losses, this penalty
explicitly permits slightly higher-than-normal loudness in the
mid-frequencies, where intelligibility yield is highest. However, it
aggressively decelerates loudness growth at spectral extremes.
...
Log-Frequency $C_(v a l s)$ Interpolation]: Modulating frequency-shaping
arrays across discrete anchor frequencies based on user experience (new,
experienced, power). 4. #strong[Reverse-Slope Low-Frequency Attenuation
Floor]: Applying a bounded log-linear low-frequency taper down toward a
$- 10$ dB floor when low-frequency loss exceeds high-frequency loss. 5.
#strong[Slope-Dependent Low-Frequency Penalty (SD-LFP) with Profound HF
...
#strong[Severe-Loss Audibility Booster]: Providing an opt-in, bounded
linear escalation (slope = 0.15) for severe thresholds. 7.
#strong[Soft-Compression High-Frequency Desensitization]: Restricting
...
- #strong[Compression Ratio Soft Penalty ($P_(c r)$)] (Value: 3.0:1
  ...
  - #emph[Justification]: #strong[Strong empirical support]: Anchored in
...
#figure(
  align(center)[#table(
    ...
    table.header([Profile], [Median SII \[Min, Max\]], [Median Loudness
      ...
      Variance ($eta^2$)],),
    table.hline(),
    [#strong[A2]], [0.87 \[0.82, 0.88\]], [4.43 \[3.38, 4.44\]], [Anchor
    (61.1%)], [38.9%],
...
#box(image("./figures/Figure4_Sensitivity.png")) #emph[Figure 4.
...
desensitization thresholds or dynamic MPO caps). 2. #strong[Differential
...
algorithms in steeply sloping audiograms. 3. #strong[Particle Swarm
...
Absolute Error (MAE) of just $0.04$ sones.

#box(image("./figures/Figure5_BlandAltman.png")) #emph[Figure 5.
...
higher-level inputs to maintain loudness parity (#strong[Figure 1],
#strong[Figure 2]).

#figure(image("./figures/OpenNL_vs_NALNL2_SII_Grouped.png", width: 100.0%),
  caption: [
    Comparison of ANSI SII (Raw Physical Audibility) vs Desensitized SII
    for NAL-NL2 and Open-NL across 50, 65, and 80 dB SPL Inputs.
  ]
)

#figure(image("./figures/OpenNL_vs_NALNL2_Gain_Final.png", width: 100.0%),
  caption: [
    Final Insertion Gain Targets for 50, 65, and 80 dB SPL Inputs across
    standard audiometric profiles, illustrating Open-NL's multi-level
    constraint-based optimization relative to NAL-NL2 (Severe-loss
    booster set to Aggressive mode for profiles A4 and A5).
  ]
)
...
boundaries: 1. #strong[Real-Ear Measurement (REM) Verification]: Because
...
Dillon, & Munro, 2021). 2. #strong[Individualized Broadband
...
#figure(
  align(center)[#table(
    ...
    [#strong[A6]], [Mixed], [50], [55], [60], [65], [75], [80], [30 dB],
...
categories, $L_(g a i n)$ is reduced by 10 dB).]
...
+ #strong[Physiological Loudness Ceiling Penalty]
  ($lambda_(l o u d) = 2000.0$): #emph[\(Note: While gain-based
  penalties in this framework are squared to strongly penalize large
  deviations, the loudness penalty is explicitly linear. This is a
  deliberate choice: because sones inherently represent a compressive
  power-law transformation of physical acoustic energy, a linear penalty
  in the sone domain naturally exerts an exponentially growing
  restriction on the underlying insertion gain. Squaring the sone error
  introduces severe mathematical stiffness and destabilizes the simplex
  gradient.)] where $upright("Sones")_(M G 04)$ is the Moore & Glasberg
