...
\end{equation}
This boost is applied exclusively at the 2000 Hz anchor band, fading out smoothly towards the adjacent anchor frequencies (1000 and 3000 Hz).
...
\end{equation}
Because this module was intended to forcefully compress targets for severe thresholds, it directly competes with the Severe-Loss Booster (Section II.D). However, a critical mathematical limitation exists: because the $L_{gain}$ limit (Equation 6) rises at 1.0 dB/dB for thresholds above 60 dB HL, while the base target (Equation 1) rises at only 0.61 dB/dB, the ceiling grows significantly faster than the target. Consequently, $G_{base}$ never mathematically exceeds $L_{gain}$ for severe-to-profound configurations. Under the equations as written, $Excess$ (Equation 7) remains zero, meaning this high-frequency desensitization module fails to engage where conceptually expected. If the calculated target gain ($G_{65}$) were to exceed this dynamic limit, the excess gain ($Excess$) would be quantified:
\begin{equation}
Excess = \max\left(0, G_{65} - L_{gain}\right)
\end{equation}
This excess gain is then softly compressed at up to a 2:1 ratio (achieving a full 2:1 compression only in the fully-weighted high-frequency region, with gentler compression elsewhere). The algorithm computes a sloping factor ($S_{factor}$) based on the difference between the specific frequency threshold and the best low-frequency threshold ($\text{HTL}_{bestlow}$), and applies a high-frequency weight ($W_{hf}$) fading in linearly from 2000 Hz to 4000 Hz:
\begin{equation}
...
To test the necessity of the cascade execution order, an open-testbed experiment was conducted on the A5 profile by reversing the sequence of the Severe-Loss Booster and the High-Frequency Desensitization soft-limiter. Interestingly, this scrambling produced zero change in the final seed output. An analysis of the computational trace revealed that because the $L_{gain}$ dynamic ceiling scales so aggressively (1.0 dB/dB) for profound thresholds, it reaches 83 dB for a 98 dB HL threshold, ensuring the base gain never crosses the threshold required to trigger soft compression. Consequently, the desensitizer offers zero resistance to the additive booster, exacerbating the loudness overshoot prior to optimization. 
...
Shen, Y., Yun, D., & Liu, Y. (2020). "Individualized Estimation of the Speech Intelligibility Index for Short Sentences: Test-Retest Reliability," The Journal of the Acoustical Society of America.
...
Yoho, S. E., & Bosen, A. K. (2019). "Individualized Frequency Importance Functions for Listeners With Sensorineural Hearing Loss," The Journal of the Acoustical Society of America.
