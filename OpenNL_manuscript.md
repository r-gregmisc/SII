...
To rule out profile-specific overfitting and ensure the C++ translation remains robust across the entire physiological parameter space, a secondary large-scale validation was conducted against 50 randomly generated audiograms (with thresholds uniformly sampled between 0 and 100 dB HL, evaluated at random input levels between 50 and 90 dB SPL). Bland-Altman analysis revealed a systematic scaling offset (Mean Bias = 5.61 sones, 95% LoA = $[-2.24, 13.45]$ sones), which is expected because the native C++ engine integrates a continuous steady-state broadband spectrum while the Octave AMT simulation (`bramslow2004`) processes a discrete 13-component time-domain multisine signal. Crucially, despite this absolute scaling difference between continuous and discrete evaluation paradigms, the structural correlation between the models was nearly perfect (Pearson's $r = 0.997$). This extremely high correlation confirms that the internal C++ optimization surface preserves the precise topographical gradients of canonical time-domain loudness models globally across arbitrary sensorineural profiles, ensuring that the target differences observed in Section III represent genuine algorithmic behavior rather than internal model variance. For mixed and conductive profiles (A6, A7), direct AMT benchmarking was omitted because canonical AMT lacks native air-bone gap parameters, whereas our C++ engine algorithmically extends the model to treat the conductive component as a linear pre-cochlear attenuator, in accordance with standard audiological principles (Dillon, 2012).
