...
calc_loudness <- function(gain_tgt, htl, cond, target_level, bin) {
  ...
  loudness_res <- calculate_loudness_cpp(
    ...
    HLohcdB0 = htl - cond, HLihcdB0 = rep(0, 6), Binaural = bin
  )
...
cat("NAL-NL2 A1 (Monaural): ", calc_loudness(nal_tgt, htl, cond, 65, 0), "\n")
cat("NAL-NL2 A1 (Binaural): ", calc_loudness(nal_tgt, htl, cond, 65, 1), "\n")

res <- open_nl(speech = 65, threshold = htl, freq = c(250, 500, 1000, 2000, 4000, 8000), loss = cond)
op_tgt <- res$gain
cat("Open-NL A1 (Monaural): ", calc_loudness(op_tgt, htl, cond, 65, 0), "\n")
cat("Open-NL A1 (Binaural): ", calc_loudness(op_tgt, htl, cond, 65, 1), "\n")
